...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt = $conn->prepare('INSERT INTO employees (employee_code,full_name,department_id,email,phone,status,shift_id) VALUES (?,?,?,?,?,?,?)');
        $stmt->bind_param('ssisssi', $code, $name, $department, $email, $phone, $status, $shift);
        $stmt->execute();
        redirectWithFlash('/admin/employees.php', 'Employee created.');
    }
} catch (Throwable $e) {
...
$rows = $conn->query('SELECT e.*,d.name department,s.shift_name FROM employees e LEFT JOIN departments d ON d.id=e.department_id LEFT JOIN shifts s ON s.id=e.shift_id ORDER BY e.id DESC');
$notice = consumeFlash();
...
<div class="panel form-panel">
    <form method="post" data-ajax-form><input type="hidden" name="csrf_token" value="<?= e(csrfToken()) ?>"><input type="hidden" name="action" value="<?= $edit ? 'update' : 'create' ?>"><?php if ($edit): ?><input type="hidden" name="id" value="<?= (int)$edit['id'] ?>"><?php endif; ?>
        <div class="form-grid"><label>Employee code<input name="employee_code" required maxlength="50" value="<?= e($edit['employee_code'] ?? '') ?>"></label><label>Full name<input name="full_name" required maxlength="150" value="<?= e($edit['full_name'] ?? '') ?>"></label><label>Department<select name="department_id">
