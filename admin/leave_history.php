...
$stmt->execute(); $applications = $stmt->get_result();
if ($employeeId > 0 && $month !== '') {
...
$transactions->execute(); $history = $transactions->get_result();
$applicationGroups = [];
...
?>
<div class="page-heading"><div><p class="eyebrow">AUDITABLE LEAVE RECORDS</p><h2>Employee leave history</h2><p class="muted">Filter applications and transactions by employee, month, and year.</p></div><form method="get" class="filter-card"><label>Employee<select name="employee_id"><option value="0">All employees</option><?php while($employee=$employees->fetch_assoc()): ?><option value="<?= (int)$employee['id'] ?>" <?= $employeeId===$employee['id']?'selected':'' ?>><?= e($employee['employee_code'].' - '.$employee['full_name']) ?></option><?php endwhile; ?></select></label><label>Month and year<input type="month" name="month" value="<?= e($month) ?>"></label><button class="btn btn-primary">Apply filter</button></form></div>
<?php if (!$applicationGroups): ?><div class="panel empty-state">No leave applications found for the selected filters.</div><?php endif; ?>
<div class="employee-card-grid"><?php foreach ($visibleApplications as $key => $items): [$code, $name] = explode('|', $key, 2); ?><section class="employee-leave-card"><div class="employee-card-header"><div class="employee-avatar"><?= e(strtoupper(substr($name, 0, 1))) ?></div><div><h3><?= e($name) ?></h3><small><?= e($code) ?></small></div><span class="status-pill status-leave"><?= count($items) ?> application(s)</span></div><div class="table-wrap"><table><thead><tr><th>Type</th><th>Dates</th><th>Days</th><th>Status</th><th>Reason</th></tr></thead><tbody><?php foreach ($items as $row): ?><tr><td class="leave-code"><?= e($row['leave_type_code']) ?></td><td><?= e($row['from_date'].' → '.$row['to_date']) ?></td><td><?= e((string)$row['days_requested']) ?></td><td><?= e(ucfirst($row['status'])) ?></td><td><?= e((string)$row['reason']) ?></td></tr><?php endforeach; ?></tbody></table></div></section><?php endforeach; ?></div>
...
<div class="section-heading"><div><h2>Balance transactions</h2><p class="muted">Showing <?= e($selectedEmployeeLabel) ?> · <?= e($selectedPeriodLabel) ?>.</p></div></div>
...
<div class="employee-card-grid"><?php foreach ($visibleTransactions as $key => $items): [$code, $name] = explode('|', $key, 2); ?><section class="employee-leave-card compact"><div class="employee-card-header"><div class="employee-avatar"><?= e(strtoupper(substr($name, 0, 1))) ?></div><div><h3><?= e($name) ?></h3><small><?= e($code) ?></small></div></div><div class="table-wrap"><table><thead><tr><th>Type</th><th>Transaction</th><th>Amount</th><th>Reference</th><th>Date</th></tr></thead><tbody><?php foreach ($items as $row): ?><tr><td class="leave-code"><?= e($row['leave_type_code']) ?></td><td><?= e(ucwords(str_replace('_',' ',$row['transaction_type']))) ?></td><td><?= e((string)$row['amount']) ?></td><td><?= e((string)$row['reference_type']) ?></td><td><?= e($row['created_at']) ?></td></tr><?php endforeach; ?></tbody></table></div></section><?php endforeach; ?></div>
